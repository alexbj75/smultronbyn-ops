...
// --- Startup-validering ---
const requiredVars = ['DB_HOST', 'DB_PORT', 'DB_NAME', 'DB_USER', 'DB_PASS',
...
if (missing.length > 0) {
  ...
  console.error('[STARTUP] Backuper kommer att misslyckas. Kontrollera parameter store smultronbynops.');
}
...
async function runBackup() {
  ...
  try {
    ...
    if (!DB_HOST || !DB_NAME || !DB_USER || !DB_PASS) {
      throw new Error('Databasvariabler saknas (DB_HOST/DB_NAME/DB_USER/DB_PASS). Avbryter.');
    }
    ...
    console.error(`[${new Date().toISOString()}] BACKUP MISSLYCKADES:`, err.message || err);
  } finally {
...
async function cleanupOldBackups(prefix, keepCount) {
  try {
    ...
    console.error(`Retention-rensning misslyckades for ${prefix}:`, err.message || err);
  }
}
